...
$itemsJson = json_encode(array_values(array_map(static function ($it) {
    ...
    if ($itemType === 'spare_part') {
        $label = trim(($it['spare_category_name'] ?? '') . ' — ' . ($it['spare_part_name'] ?? ''));
        if (!empty($it['spare_part_number'])) {
            $label .= ' (' . $it['spare_part_number'] . ')';
        }
    } else {
  ...
  Alpine.data('poShow', () => ({
    ...
    initReceive() {
      this.allocations = this.poItems.map(it => ({
        ...
        rows: it.item_type === 'spare_part'
          ? [{ warehouse_id: 0, quantity: it.pending }]
          : [{ warehouse_id: this.warehouses[0]?.id || '', quantity: it.pending }],
      }));
...
<div x-data="poShow()">
  <div class="toolbar">
    ...
    <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
      ...
      <a class="btn btn-outline" href="<?= url('spare-parts') ?>">Spare Parts</a>
    </div>
  </div>
    ...
    <div class="table-wrap">
      <table class="data">
        ...
        <?php foreach ($items as $i => $it): ?>
          ...
          <tr>
            <td><?= $i + 1 ?></td>
            <td><?= $itemType === 'spare_part' ? 'Spare part' : 'Vehicle' ?></td>
            <td><?= e($it['hsn_code']) ?></td>
            <td>
              <?php if ($itemType === 'spare_part'): ?>
                <strong><?= e($it['spare_part_name'] ?? '—') ?></strong>
                <?php if (!empty($it['spare_category_name'])): ?>
                  <span class="muted">(<?= e($it['spare_category_name']) ?>)</span>
                <?php endif; ?>
                <?php if (!empty($it['spare_part_number'])): ?>
                  <div class="muted" style="font-size:0.78rem;"><?= e($it['spare_part_number']) ?></div>
                <?php endif; ?>
              <?php else: ?>
            ...
            <td><?= e($itemType === 'vehicle_variant' ? ($it['color'] ?? '—') : '—') ?></td>
        ...
        <tfoot>
          <tr>
            <td colspan="10" style="text-align:right;"><strong>Totals</strong></td>
        ...
        <?php if (!empty($receiptLines[(int)$r['id']])): ?>
          <ul style="margin:0.5rem 0 0;padding-left:1.2rem;font-size:0.88rem;">
            <?php foreach ($receiptLines[(int)$r['id']] as $ln): ?>
              <li>
                <?= (int)$ln['quantity'] ?> ×
                <?php if (($ln['item_type'] ?? 'vehicle_variant') === 'spare_part'): ?>
                  <?= e($ln['spare_part_name']) ?>
                  → <strong>Spare parts stock</strong>
                <?php else: ?>
                  <?= e($ln['vehicle_name']) ?> — <?= e($ln['variant_name']) ?>
    ...
    <div class="modal modal-lg">
      <form method="post" action="<?= url('purchase-orders/' . (int)$po['id'] . '/receive') ?>">
        ...
        <div class="modal-header">
          <h3 class="modal-title">Receive stock</h3>
          <button type="button" class="btn btn-sm btn-outline" @click="receiveOpen=false">Close</button>
        </div>
        <div class="modal-body" style="max-height:70vh;overflow-y:auto;">
          <p class="muted" style="margin-top:0;">Vehicle lines are split across warehouses. Spare parts/batteries update spare parts stock directly.</p>
            ...
            <div class="card" style="padding:0.75rem;margin-bottom:0.75rem;background:var(--surface-2);">
              <div style="display:flex;justify-content:space-between;align-items:start;gap:0.5rem;margin-bottom:0.5rem;">
                <div>
                  ...
                  <div class="muted" style="font-size:0.8rem;">
                    <span x-text="item.item_type === 'spare_part' ? 'Spare part' : 'Vehicle'"></span>
                    · Pending: <span x-text="item.pending"></span> · Allocating: <span x-text="itemAllocated(item)"></span>
                  </div>
                </div>
                <button class="btn btn-sm btn-outline" type="button" @click="addWhRow(itemIdx)" x-show="item.item_type !== 'spare_part'">+ Warehouse</button>
              </div>
              <template x-for="(row, rowIdx) in item.rows" :key="rowIdx">
                <div style="display:flex;gap:0.5rem;align-items:end;margin-bottom:0.4rem;">
                  ...
                  <input type="hidden" :name="'allocations['+itemIdx+'_'+rowIdx+'][warehouse_id]'" :value="item.item_type === 'spare_part' ? 0 : row.warehouse_id">
                  <div class="form-group" style="margin:0;flex:1;" x-show="item.item_type !== 'spare_part'">
                    <label x-show="rowIdx===0">Warehouse</label>
                    <select class="form-control" x-model="row.warehouse_id" :required="item.item_type !== 'spare_part'">
                  ...
                  <div class="form-group" style="margin:0;flex:1;" x-show="item.item_type === 'spare_part'">
                    <label x-show="rowIdx===0">Destination</label>
                    <div class="form-control" style="background:var(--surface);">Spare parts stock</div>
                  </div>
                  ...
                  <button class="btn btn-sm btn-danger" type="button" @click="removeWhRow(itemIdx, rowIdx)" x-show="item.item_type !== 'spare_part' && item.rows.length > 1" style="margin-bottom:0.35rem;">×</button>
                </div>
              </template>
            </div>
          </template>
